...
for(const curved of [false,true]) for(const width of [1440,390]) test(`opening-only ${curved?'curved':'straight'} group moves at ${width}px`,async({page},testInfo)=>{
 ...
 await page.mouse.move(x,y);await page.mouse.down();await page.mouse.move(x+50,y,{steps:8});await page.mouse.up();
 const moved=await exported();expect(moved.doors[0].position).toBeGreaterThan(before.doors[0].position);expect(moved.windows[0].position).toBeGreaterThan(before.windows[0].position);
 ...
 if(!curved)expect(moved.windows[0].position-moved.doors[0].position).toBeCloseTo(.4);
 ...
 await testInfo.attach(`opening-group-${curved}-${width}`,{body:await page.screenshot(),contentType:'image/png'});
});
