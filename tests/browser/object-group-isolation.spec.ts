...
for(const kind of ['furniture','columns','stairs','entourage','doors','windows','walls']) {
  test(`Ctrl/Cmd-click isolates grouped ${kind}`,async({page})=>{
    ...
    await shiftClick(0);await shiftClick(200);
    await expect(page.getByText('2 selected',{exact:true}).first()).toBeVisible();
    await save.press('ControlOrMeta+g');const before=await exported();expect(before.groups).toHaveLength(1);
    for(const activeGroup of [true,false]) {
      if(!activeGroup)await save.press('Escape');
      await fitted();const target=await page.evaluate(()=>{
        const a=(window as any).__references['Reference A'],b=(window as any).__references['Reference B'],scale=(b.x-a.x)/100;
        return {x:a.x+200*scale,y:a.y+200*scale};
      });
      await page.keyboard.down('ControlOrMeta');await page.mouse.click(target.x,target.y);await page.keyboard.up('ControlOrMeta');
      await expect(page.getByText('2 selected',{exact:true})).toHaveCount(0);
      expect(await exported()).toEqual(before);
      await save.press('Delete');expect(await exported()).toEqual({...before,[kind]:before[kind].filter((item:any)=>item.id!=='item1'),groups:[]});
