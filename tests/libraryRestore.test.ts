...
import { mockStorage, rawRecords, putRaw, failWrites, libraryBackup } from './fixtures/projectStore';
...
it('previews a current backup without reaching the server or changing the editor', () => {
...
it('restores independent project, thumbnail and remapped history without overwriting the existing entry', async () => {
  const project = roomProject(), old = structuredClone(project), store = createServerStore();
...
it.each(['__proto__', 'constructor', 'toString', 'id?with#punctuation & spaces'])('restores a bare map containing the source ID %s safely', async id => {
  ...
  expect((await createServerStore().load(restored.projects[0].id))!.floors).toEqual(project.floors);
});

it('never selects a source, existing, or already-restored ID for another copy', async () => {
  const a = roomProject(), b = roomProject(), existing = roomProject(); await createServerStore().save(existing);
  ...
  expect(await createServerStore().list()).toHaveLength(3);
});
...
it('carries prior recovery archives flat and preserves colliding archive contents', async () => {
  await putRaw('meta', 'existing', '{existing raw recovery');
...
it.each(['projects', 'thumbnails', 'history', 'meta'] as const)('rolls back the whole restore after a %s write failure and supports retry', async failedStore => {
  const source = roomProject(), existing = roomProject(); await createServerStore().save(existing);
...
it('shares a pending restore and never repeats an already committed preview', async () => {
  ...
  expect(await preview.restore()).toBe(result); expect(await createServerStore().list()).toHaveLength(1);
});

it('rejects a cancelled restore before reaching the server', async () => {
...
it('shows damaged saved entries alongside restored projects and keeps their bytes downloadable', async () => {
  ...
  const store = createServerStore(), list = await store.list();
...
it('preserves embedded images and unknown fields in a multi-megabyte project', async () => {
...
it.each(['{broken', 'null', '[]', '{"format":"openplan3d-library","version":2,"projects":{}}', '{"format":"openplan3d-library","version":1}', '{"entry":42}', '{"same":"one","same":"two"}', '{"same":"one","\\u0073ame":"two"}'])('rejects an invalid or ambiguous backup before reaching the server: %s', raw => {
  const reached = vi.spyOn(globalThis, 'fetch');
  expect(() => prepareLibraryRestore(raw)).toThrow(); expect(reached).not.toHaveBeenCalled();
});
...
it('rejects mismatched project IDs and malformed geometry while retaining each raw entry', async () => {
  ...
  await preview.restore(); expect(await createServerStore().list()).toEqual([]);
  const backup = JSON.parse(await libraryBackup());
